...
public final class NetworkAgent implements IResetEvent {
	...
	public void broadcastMessage(final Envelope env) throws AdaptinetException {
		...
		if (bConnected == false) {
			AdaptinetException adaptinetex = new AdaptinetException(BaseException.SEVERITY_ERROR,
					BaseException.GEN_MESSAGE);
			adaptinetex.logMessage("Can not broadcast message while not connected");
			throw adaptinetex;
		}
		root.broadcastMessage(env.getHeader().getMessage(), env.getBody().getcontentArray());
	}

	public void broadcastMessage(final Message message, final Object[] args) throws AdaptinetException {
