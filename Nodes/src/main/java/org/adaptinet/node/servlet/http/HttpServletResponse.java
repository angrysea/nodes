...
import org.adaptinet.nnode.http.HTTP;
import org.adaptinet.nnode.http.Response;
...
public class HttpServletResponse {
	...
	public void setDateHeader(String name, long date) {
		...
		headers.setProperty(name, df.format(d));
	}
	...
	public void setIntHeader(String name, int value) {
		headers.setProperty(name, Integer.toString(value));
	}
	...
	protected void writeHeaders(DataOutputStream out) {
		try {
			...
			while (it.hasNext()) {
				...
				out.write(": ".getBytes());
				out.write(headers.getProperty(key).getBytes());
			}
	...
	private String cookieToString(Cookie c) {
		try {
			...
			cookie += URLEncoder.encode(c.getName());
			cookie += "=";
			cookie += URLEncoder.encode(c.getValue());
			if (c.getMaxAge() > 0)
				cookie += ";EXPIRES=" + getExpires(c.getMaxAge());
			if (c.getDomain() != null)
