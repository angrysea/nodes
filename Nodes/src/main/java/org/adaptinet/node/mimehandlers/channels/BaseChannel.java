...
public abstract class BaseChannel implements Channel {
	...
	public Controller getController(String serviceName)
			throws InstantiationException, IllegalAccessException {
		Class controllerClass = ServicesConfig.getController(serviceName);
	...
	public void writeDebugFile(String fileName, Logger logger, String data,
			boolean bInputFile) {
		...
		if (DUMP) {
			...
			try {
				...
				if (bInputFile)
					logger.info("Request: Dumped to file : "
							+ file.getAbsolutePath());
				else
					logger.info("Response: Dumped to file : "
							+ file.getAbsolutePath());

			} catch (Exception e) {
				logger.info("CANVAS SVC: Error writing to debug file: "
						+ filename, e);
			} finally {
				if (os != null) {
					try {
