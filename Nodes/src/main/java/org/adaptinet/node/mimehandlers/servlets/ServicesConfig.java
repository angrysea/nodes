...
public class ServicesConfig {
	...
	public ServicesConfig(HttpServletConfig sc, ServletContext context) {
		...
		if (services == null) {
			String remoteConfig = sc.getInitParameter(REMOTINGCONFIG);
			if (remoteConfig != null && remoteConfig.length() > 0) {
				InputStream is = context.getResourceAsStream(remoteConfig);
				if (is != null) {
					...
					try {
						for (Destination destination : in.destinationArray()) {
							...
							if (id != null && id.length() > 0) {
								String className = destination.getProperty()
								...
								if (className != null && className.length() > 0) {
									try {
										...
										error(
												"Service Class not found check remoting-config.xml. ",
												e);
									}
									String packageName = destination
											.getProperty().getSource()
											.getPackageName();
									if (packageName != null
											&& packageName.length() > 0) {
										packages.put(id, destination
												.getProperty().getSource()
												.getPackageName());
										Map<String, Class> methods = new HashMap<String, Class>();
										for (ControllerMethod method : destination
												.getProperty().getSource()
											...
											if (method.getRequestName() != null) {
												try {
													...
													error(
															"Class not found check remoting-config.xml. ",
															e);
												}
											}
										}
						...
						error(
								"Remote-config file is invalid. Unable to parse. ",
								e);
					}
				} else {
					error("Unable to open remoting configuration file: "
							+ remoteConfig);
				}
			} else {
	...
	public static Class getRequest(String serviceName, String methodName) {
		Map<String, Class> methods = controllerMethods.get(serviceName);
