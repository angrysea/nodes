...
public class Mime_Binary implements MimeBase {
	...
	public ByteArrayOutputStream process(IServer server, Request request) {
		...
		try {
			...
			if (s != null && s.length > HANDLER_POS) {
				...
				if (name != null && name.length() > 0) {
					...
					if (processor != null) {
						...
						final AMGException exMessage = new AMGException(BaseException.SEVERITY_SUCCESS,
				...
				final AMGException exMessage = new AMGException(BaseException.SEVERITY_ERROR,
			...
			exMessage = new AMGException(BaseException.SEVERITY_SUCCESS, BaseException.GEN_MESSAGE);
