package org.amg.node.mimehandlers.servlets;
...
import org.amg.node.mimehandlers.channels.Channel;
import org.amg.node.mimehandlers.utils.RestPropertiesUtil;
...
import org.amg.node.servlet.http.xml.XMLHttpServlet;
...
public class ChannelServlet extends XMLHttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse resp) {
		...
		if (s != null && s.length > CONTROLLER_POS) {
			...
			if (controllerName != null && controllerName.length() > 0 && methodName != null
					&& methodName.length() > 0) {
				try {
					Class<?> channelClass = Class.forName("com.db.canvas.channels." + channelName + "Channel");
					...
					// logger.error("Processing request, ex=" + e.getMessage(),
					// e);
					setError(resp, e.getMessage());
				}
			} else {
				// logger.error("Malformed URL either controller name or method
				// is missing.");
			}
		} else {
	...
	private void setError(HttpServletResponse resp, String msg) {
		String result = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><status><servlet>" + this.getClass().getName()
				+ "</servlet><code>1</code><desc>" + msg + "</desc><timestamp>"
	...
	public void logTransaction(HttpServletRequest request) {
		...
		// logger.info("logResponseHeader reade from properties file >>
		// "+logResponseHeader);
		...
		if (logResponseHeader != null && "true".equals(logResponseHeader)) {
			...
			writer.println(request.getLocalesAsString());

			Set<String> names = request.getParameterNames();
			for (String name : names) {
				writer.print("         parameter " + name + "=");
				writer.println(Arrays.asList(request.getParameterValues(name)).stream().collect(Collectors.joining()));
			}
	...
	public void sendErrorMessage(String msg, Exception e, HttpServletResponse response) {
		...
		try {
			...
			// logger.error("XMLSVC: Sending error message: " + msg + ",
			// origex="
