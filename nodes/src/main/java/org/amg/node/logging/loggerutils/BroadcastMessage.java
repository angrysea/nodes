package org.amg.node.logging.loggerutils;
...
public class BroadcastMessage implements Serializable {
	...
	private void init(int facility, int severity) throws LoggerException {
		if (facility >= AMGException.MIN_FACILITY && facility <= AMGException.MAX_FACILITY) {
			m_iFacility = facility;
		} else {
			throw new LoggerException(1, 1, "Invalid Facility");
		}

		if (severity >= AMGException.MIN_SEVERITY && severity <= AMGException.MAX_SEVERITY) {
