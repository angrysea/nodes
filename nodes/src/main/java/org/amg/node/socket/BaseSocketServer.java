	...
	static final public BaseSocketServer createInstance(String type,
		...
		try {
			if (type.startsWith("PROXY")) {
				c = (Class<BaseSocketServer>)Class.forName("org.amg.node.socket.ProxySocketServer");
				baseType = type;
			} else if (type.startsWith("CALLBACK")) {
				c = (Class<BaseSocketServer>)Class.forName("org.amg.node.socket.CallbackSocketServer");
				baseType = type;
				...
				c = (Class<BaseSocketServer>)Class
						.forName("org.amg.sdk.socket.SecureHttpSocketServer");
				baseType = type.substring(3);
			} else if (type.startsWith("CUSTOM")) {
				c = (Class<BaseSocketServer>)Class.forName(serverclass);
				baseType = type;
			} else {
				c = (Class<BaseSocketServer>)Class.forName("org.amg.node.socket.HttpSocketServer");
				baseType = type;
			}
