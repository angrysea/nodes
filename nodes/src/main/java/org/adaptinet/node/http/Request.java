...
public class Request {
	...
	public Response processRequest(IServer server, OutputStream out) {
		...
		try {
			...
			case XML_TEXT:
				mime = (MimeBase) Class.forName("org.adaptinet.node.mimehandlers.MimeXML_" + type).newInstance();
				contentType = HTTP.contentTypeXML;
				break;

			case DATA_TEXT:
				mime = (MimeBase) Class.forName("org.adaptinet.node.mimehandlers.MimeCommand_" + type).newInstance();
			...
			case HTML_TEXT:
				if (type == null)
					type = "Processor";
				mime = (MimeBase) Class.forName("org.adaptinet.node.mimehandlers.MimeHTML_HTTP").newInstance();
			...
			case XML_WS:
				mime = (MimeBase) Class.forName("org.adaptinet.node.mimehandlers.MimeXML_WS").newInstance();
				mime.init(url, server);
				respond = true;
				break;

			case MIME_BINARY:
				mime = (MimeBase) Class.forName("org.adaptinet.node.mimehandlers.Mime_Binary").newInstance();
				mime.init(url, server);
